use std::ptr;
use log::info;
use crate::config;

fn reverse_bits(byte: u8) -> u8 {
    let mut result = 0;
    let mut b = byte;
    for _ in 0..8 {
        result = (result << 1) | (b & 1);
        b >>= 1;
    }
    result
}

pub fn decrypt_rtv(encrypted_bytes: &[u8]) -> Vec<u8> {
    encrypted_bytes
        .iter()
        .map(|&b| !b)
        .map(reverse_bits)
        .collect()
}

pub fn encrypt_rtv(decrypted_bytes: &[u8]) -> Vec<u8> {
    decrypted_bytes
        .iter()
        .map(|&b| reverse_bits(b))
        .map(|b| !b)
        .collect()
}

pub fn check_sum(data: &[u8]) -> [u8; 2] {
    const TABLE: [u8; 512] = [
        
    ];
    let mut sum = [0u8; 2];
    let mut index = 0;
    for i in 0..data.len() {
        index = sum[0] ^ data[i];
        sum[0] = sum[1] ^ TABLE[index as usize * 2 + 1];
        sum[1] = TABLE[index as usize * 2];
    }
    sum
}

pub fn decode_config(file_path: &str) {
    let mut file_bytes = std::fs::read(file_path).unwrap();
    let mut dst = vec![0u8; 2048];
    decode_dat(&mut file_bytes, dst.as_mut_slice());

    let mut config = String::new();
    for i in 0..dst.len() {
        if dst[i] == 0 {
            break;
        }
        config.push(dst[i] as char);
    }
    info!("解密成功，原文如下：\n{}", config);
}

fn decode_dat(src: &mut [u8], dst: &mut [u8]) {
    let mut tmp = vec![0x20u8; 0x8000];
    for i in 0..src.len() {
        src[i] = 255 - src[i];
    }

    let mut m = 0;
    let mut n = 0;
    let mut d = 0;
    let mut si = 0;
    let mut di = 0;
    let mut s;

    s = src[si];
    si += 1;
    loop {
        if si >= src.len() {
            return;
        }
        for i in 0..8 {
            d = 1 << i;
            if (s & d) == 0 {
                d = src[si];
                si += 1;
                if si >= src.len() {
                    return;
                }
                tmp[(m << 7) ^ n] = d;
            } else {
                d = tmp[(m << 7) ^ n];
            }
            dst[di] = d;
            di += 1;
            if di >= dst.len() {
                return;
            }
            m = n;
            n = d as usize;
        }
        s = src[si];
        si += 1;
    }
}

#[test]
fn test_vendor_crypt() {
    let encrypted_bytes = [
        0xFF,0xFF,0x37,0x77,0x7F,0x9F,0xFF,0xFF,0xD9,0x13,0xFF,0xFF,0x37,0x77,
        0x7F,0x9F,0xFF,0xFF,0xF7,0x2B,0xFF,0xFF,0x37,0x77,0x7F,0x3F,0xFF, 0x00, 0x00, 0x00
    ];
    let decrypted_bytes = decrypt_rtv(&encrypted_bytes);

    println!("Encrypted Bytes: {:?}", encrypted_bytes);
    println!("Decrypted Bytes: {:?}", decrypted_bytes);
    println!("Decrypted Bytes: {:?}", hex::encode(decrypted_bytes));
}

#[test]
fn test_check_sum() {
    let data = [00 ,00 ,0xff ,0xff];
    let sum = check_sum(&data);
    println!("Check Sum: {:?}", sum);
}

#[test]
fn test_unknown_data() {
    let salt = "!jierui9002pmsus".as_bytes();
    let mut data = [0u8; 32];
    // copy salt to data
    unsafe {
        ptr::copy_nonoverlapping(salt.as_ptr(), data.as_mut_ptr().add(16), salt.len());
    }

    let out = md5::compute(data);
    println!("{:?}", hex::encode(out.0));
}